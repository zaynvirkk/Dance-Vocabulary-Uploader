...
export async function uploadEntries(entries: Entry[]) {
    for (const entry of entries) {
        ...
        if (entry.video.size > 100 * 1024 * 1024) {
            throw new FirebaseError(`The video "${entry.video.name}" is larger than 100MB. Please upload a smaller file.`);
        }
        ...
        if (!titleQuerySnapshot.empty) {
            throw new FirebaseError(`A dance move with the title "${entry.title}" already exists.`);
        }
        ...
        try {
            ...
            throw new FirebaseError(`A video with the name "${entry.video.name}" already exists.`);
        } catch (error) {
...
export async function uploadEntryWithVideo(entry: Entry) {
    ...
    try {
        ...
        throw new FirebaseError(`A thumbnail with the name "${entry.thumbnail.name}" already exists.`);
    } catch (error) {
        if (error instanceof StorageError && error.code !== 'storage/object-not-found') {
            throw error;
        }
